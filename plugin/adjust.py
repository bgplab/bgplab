...
def adjust_topology(a_entry: Box, topology: Box) -> None:
  ...
  for n_name in get_a_list(a_entry,'nodes'):      # Iterate over nodes to check
    ...
    if missing_features(a_entry,n_data,topology): # ... check for missing features
      miss_list.append(n_data.name)               # ... and collect non-compliant node names
  ...
  m_text = a_entry.get('message','')              # Do we have to adjust the lab message?
  if m_text:
    if 'message' not in topology:
      topology.message = ''

    m_warning = "\n".join(
                    strings.wrap_text_into_lines(
                      strings.eval_format(m_text,n_data + {'node_list': ','.join(miss_list)}),
                      width=72))
    topology.message += "\n"+m_warning+"\n"
